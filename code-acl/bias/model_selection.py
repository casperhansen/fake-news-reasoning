...
def _run_thread_job(model_params):  # pragma: no cover
    try:
        ...
        elif type(scoring) in [metrics._scorer._PredictScorer, metrics._scorer._ProbaScorer] \
            or metrics._scorer._PredictScorer in type(scoring).__bases__ \
            or metrics._scorer._ProbaScorer in type(scoring).__bases__:
...
class GridSearch(BaseEstimator):
    ...
    def fit(
        ...
        if validation_data_exists:
            ...
            print("model fit time (s)", time.time() - start_time)
        else:
